...
object QuantileMethods {
  ...
  /** Returns the average number of data points between true and estimated quantiles.
    ...
    */
  def misclassifiedPointsError(
      sortedData: Array[Double],
      trueQuantiles: Array[Double],
      estQuantiles: Array[Double]
  ): Double = {
    val numQuantiles = trueQuantiles.length
    val totalMissed = (0 until numQuantiles)
      .map { qIdx =>
        abs(
          sortedData.count(_ > trueQuantiles(qIdx)) - sortedData.count(
            _ > estQuantiles(qIdx)
          )
        )
      }
